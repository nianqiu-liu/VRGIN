...
namespace VRGIN.Core
{
	public class VRGUI : ProtectedBehaviour, IScreenGrabber
	{
		...
		protected override void OnAwake()
		{
			...
			{
				base.transform.gameObject.AddComponent<FastGUI>();
				base.transform.gameObject.AddComponent<SlowGUI>();
			}
			...
			_VRGUICamera.transform.SetParent(base.transform, false);
		...
		{
			_VRGUICamera.targetTexture = uGuiTexture;
			foreach (Canvas item in (_Registry.Keys as ICollection<Canvas>).Where((Canvas c) => c).ToList().Where(IsUnprocessed))
			{
				if (VR.Interpreter.IsIgnoredCanvas(item))
				{
					continue;
				}
				VRLog.Info("Add {0} [Layer: {1}, SortingLayer: {2}, SortingOrder: {3}, RenderMode: {4}, Camera: {5}, Position: {6} ]", item.name, LayerMask.LayerToName(item.gameObject.layer), item.sortingLayerName, item.sortingOrder, item.renderMode, item.worldCamera, item.transform.position);
				...
				if (((1 << item.gameObject.layer) & VR.Context.UILayerMask) == 0)
				{
					...
					Transform[] componentsInChildren = item.gameObject.GetComponentsInChildren<Transform>();
				...
				{
					Graphic[] componentsInChildren2 = item.gameObject.GetComponentsInChildren<Graphic>();
				...
				{
					GraphicRaycaster component = item.GetComponent<GraphicRaycaster>();
					if ((bool)component)
					{
						Object.DestroyImmediate(component);
						SortingAwareGraphicRaycaster obj2 = item.gameObject.AddComponent<SortingAwareGraphicRaycaster>();
