...
namespace CapturePanorama
{
    public static class Icosphere
    {
        ...
        {
            var mesh = BuildIcosahedron(radius);
            for (var i = 0; i < iterations; i++) Refine(mesh);
            return mesh;
        }
        ...
        {
            var mesh = new Mesh();
            var num = (float)((1.0 + Math.Sqrt(5.0)) / 2.0);
            var array = new Vector3[12]
            {
            ...
            var num2 = radius / new Vector3(1f, num, 0f).magnitude;
            for (var i = 0; i < array.Length; i++) array[i] *= num2;
            mesh.vertices = array;
