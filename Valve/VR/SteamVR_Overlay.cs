...
namespace Valve.VR
{
    public class SteamVR_Overlay : MonoBehaviour
    {
        ...
        [Tooltip("Size of overlay view.")] public float scale = 3f;

        [Tooltip("Distance from surface.")] public float distance = 1.25f;

        [Tooltip("Opacity")] [Range(0f, 1f)] public float alpha = 1f;
        ...
        private void OnEnable()
        {
            ...
            if (overlay != null)
            {
                var eVROverlayError = overlay.CreateOverlay(key, gameObject.name, ref handle);
        ...
        public void UpdateOverlay()
        {
            ...
            if (texture != null)
            {
                var eVROverlayError = overlay.ShowOverlay(handle);
                if ((eVROverlayError != EVROverlayError.InvalidHandle && eVROverlayError != EVROverlayError.UnknownOverlay) || overlay.FindOverlay(key, ref handle) == EVROverlayError.None)
                {
                    ...
                    var pvecMouseScale = default(HmdVector2_t);
        ...
        public bool ComputeIntersection(Vector3 source, Vector3 direction, ref IntersectionResults results)
        {
            var overlay = OpenVR.Overlay;
            if (overlay == null) return false;
            var pParams = default(VROverlayIntersectionParams_t);
            ...
            var pResults = default(VROverlayIntersectionResults_t);
            if (!overlay.ComputeOverlayIntersection(handle, ref pParams, ref pResults)) return false;
