...
namespace VRGIN.Controls
{
	public class DeviceLegacyAdapter
	{
		...
		{
			for (int i = 0; i < SUPPORTED_BUTTON_ID.Length; i++)
			{
				EVRButtonId buttonId = SUPPORTED_BUTTON_ID[i];
				ulong num = SUPPORTED_BUTTON_MASK[i];
				if ((buttonMaxk & num) != 0L && GetPress(buttonId))
				{
					return true;
				}
			}
		...
		public bool GetPressDown(ulong buttonMaxk)
		{
			for (int i = 0; i < SUPPORTED_BUTTON_ID.Length; i++)
			{
				EVRButtonId buttonId = SUPPORTED_BUTTON_ID[i];
				ulong num = SUPPORTED_BUTTON_MASK[i];
				if ((buttonMaxk & num) != 0L && GetPressUp(buttonId))
				{
					return true;
				}
			}
		...
		public bool GetPressUp(ulong buttonMaxk)
		{
			...
			for (int i = 0; i < SUPPORTED_BUTTON_ID.Length; i++)
			{
				...
				{
					if (GetPress(buttonId))
					{
						return false;
					}
					if (GetPressUp(buttonId))
					{
						result = true;
					}
				}
		...
		public bool GetTouchUp(ulong buttonMaxk)
		{
			...
			for (int i = 0; i < SUPPORTED_BUTTON_ID.Length; i++)
			{
				...
				if ((buttonMaxk & num) != 0L)
				{
					if (GetTouch(buttonId))
					{
						return false;
					}
					if (GetTouchUp(buttonId))
					{
						result = true;
					}
				}
		...
		public void TriggerHapticPulse(ushort durationMicroSec = 500, EVRButtonId buttonId = EVRButtonId.k_EButton_Axis0)
		{
			...
			SteamVR_Input_Sources inputSource = _tracking.inputSource;
			float durationSeconds = (float)((int)durationMicroSec / 1000) / 1000f;
		...
		{
			SteamVR_Input_Sources inputSource = _tracking.inputSource;
			SteamVR_Action_Single_Source steamVR_Action_Single_Source = SteamVR_Actions.legacy_emulate.Axis1_1D[inputSource];
			hairTriggerPrevState = hairTriggerState;
			float axis = steamVR_Action_Single_Source.axis;
			if (hairTriggerState)
			{
				if (axis < hairTriggerLimit - hairTriggerDelta || axis <= 0f)
				{
					hairTriggerState = false;
				}
			}
			else if (axis > hairTriggerLimit + hairTriggerDelta || axis >= 1f)
			{
		...
		{
			SteamVR_Input_ActionSet_legacy_emulate legacy_emulate = SteamVR_Actions.legacy_emulate;
			SteamVR_Input_Sources inputSource = _tracking.inputSource;
