...
namespace VRGIN.Visuals
{
	...
	public class ArcRenderer : MonoBehaviour
	{
		...
		public void Update()
		{
			...
			float num = (0f - (Velocity * base.transform.forward).y) * Scale;
			float num2 = Physics.gravity.y * Scale;
			float num3 = position.y - Offset;
			float a = (Mathf.Sqrt(num * num - 2f * num2 * num3) + num) / num2;
			float b = (num - Mathf.Sqrt(num * num - 2f * num2 * num3)) / num2;
			float f = Mathf.Max(a, b);
			f = Mathf.Abs(f);
			float num4 = f / (float)VertexCount;
			for (int j = 0; j <= VertexCount; j++)
			{
				float num5 = Mathf.Clamp((float)j / ((float)VertexCount - 1f) * f + Time.time * UvSpeed % 2f * num4 - num4, 0f, f);
				list.Add(base.transform.InverseTransformPoint(position + (forward * Velocity * num5 + 0.5f * Physics.gravity * num5 * num5) * Scale));
			}
			target = base.transform.position + (forward * Velocity * f + 0.5f * Physics.gravity * f * f) * Scale;
			target.y = 0f;
