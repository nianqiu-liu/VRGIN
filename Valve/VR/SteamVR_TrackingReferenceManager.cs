...
namespace Valve.VR
{
    public class SteamVR_TrackingReferenceManager : MonoBehaviour
    {
        ...
        private void OnNewPoses(TrackedDevicePose_t[] poses)
        {
            ...
            for (var num = 0u; num < poses.Length; num++)
            {
                if (!trackingReferences.ContainsKey(num))
                {
                    var trackedDeviceClass = OpenVR.System.GetTrackedDeviceClass(num);
                    if (trackedDeviceClass == ETrackedDeviceClass.TrackingReference)
                    {
                        var trackingReferenceObject = new TrackingReferenceObject();
                        ...
                        trackingReferenceObject.gameObject.transform.parent = transform;
