...
namespace VRGIN.Helpers
{
    public class Profiler : ProtectedBehaviour
    {
        ...
        {
            if (!GameObject.Find("Profiler")) new GameObject("Profiler").AddComponent<Profiler>()._Callback = callback;
        }
        ...
        {
            var queue = (from n in UnityHelper.GetRootNodes().Except(new GameObject[1] { gameObject })
                         where !n.name.StartsWith("VRGIN") && !n.name.StartsWith("[")
                         select n).ToList();
            yield return StartCoroutine(MeasureFramerate(30));
            var startInterval = _CurrentInterval;
            ...
            while (queue.Count > 0)
            {
                ...
                yield return StartCoroutine(MeasureFramerate(30));
                obj.SetActive(true);
                var num = startInterval / _CurrentInterval;
                ...
                if (num > 1.149999976158142)
                {
                    ...
                    foreach (var component in from c in obj.GetComponents<Behaviour>()
                                              where c.enabled
                                              select c)
                    {
