...
namespace Valve.VR
{
    public static class SteamVR_Input_Source
    {
        ...
        {
            if ((int)inputSource < inputSourceHandlesBySource.Length) return inputSourceHandlesBySource[(int)inputSource];
            return 0uL;
        }

        public static SteamVR_Input_Sources GetSource(ulong handle)
        {
            if (inputSourceSourcesByHandle.ContainsKey(handle)) return inputSourceSourcesByHandle[handle];
            return SteamVR_Input_Sources.Any;
        }

        public static SteamVR_Input_Sources[] GetAllSources()
        {
            if (allSources == null) allSources = (SteamVR_Input_Sources[])Enum.GetValues(typeof(SteamVR_Input_Sources));
            return allSources;
        }
        ...
        {
            var list = new List<SteamVR_Input_Sources>();
            ...
            {
                var path = GetPath(names[i]);
                var pHandle = 0uL;
                var inputSourceHandle = OpenVR.Input.GetInputSourceHandle(path, ref pHandle);
                if (inputSourceHandle != 0) Debug.LogError("<b>[SteamVR]</b> GetInputSourceHandle (" + path + ") error: " + inputSourceHandle);
