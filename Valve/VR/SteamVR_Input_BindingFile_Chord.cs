...
namespace Valve.VR
{
    ...
    public class SteamVR_Input_BindingFile_Chord
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            {
                var steamVR_Input_BindingFile_Chord = (SteamVR_Input_BindingFile_Chord)obj;
                if (output == steamVR_Input_BindingFile_Chord.output && inputs != null && steamVR_Input_BindingFile_Chord.inputs != null && inputs.Count == steamVR_Input_BindingFile_Chord.inputs.Count)
                {
                    ...
                    {
                        if (inputs[i] == null || steamVR_Input_BindingFile_Chord.inputs[i] == null || inputs[i].Count != steamVR_Input_BindingFile_Chord.inputs[i].Count) continue;
                        for (var j = 0; j < inputs[i].Count; j++)
                        {
                            if (inputs[i][j] != steamVR_Input_BindingFile_Chord.inputs[i][j]) return false;
                        }
