	...
	public class GuiScaler
	{
		...
		public void Update()
		{
			if ((bool)_Left && (bool)_Right)
			{
				...
				Vector3 vector = _Right.position - _Left.position;
				Vector3 vector2 = _Left.position + vector * 0.5f;
		...
		{
			Vector3 normalized = (_Right.position - _Left.position).normalized;
			Vector3 vector = Vector3.Lerp(_Left.forward, _Right.forward, 0.5f);
