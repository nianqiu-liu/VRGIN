    ...
    public class VRLog
    {
        ...
        public static void Log(string text, object[] args, LogMode severity)
        {
            try
            {
                ...
                var value = string.Format(Format(text, severity), args);
                lock (_LOCK)
                {
        ...
        {
            var frame = new StackTrace(3).GetFrame(0);
            return string.Format("[{0}][{1}][{3}#{4}] {2}", DateTime.Now.ToString("HH':'mm':'ss"), mode.ToString().ToUpper(), text, frame.GetMethod().DeclaringType.Name, frame.GetMethod().Name,
